...
| `display.rightAlign` | string[] | `[]` | Elements pushed to the right edge of the merged line they belong to, padding the gap with spaces. Requires the elements to be in a `display.mergeGroups` group that actually renders on one line. Ignored when the terminal width is unknown, when every element of the group is listed, or when there is no room for the padding — each case falls back to the normal ` │ ` join. Example: `["context","usage"]` with a `["project","context","usage"]` merge group keeps the project/git block left and pins context + usage to the right. |
